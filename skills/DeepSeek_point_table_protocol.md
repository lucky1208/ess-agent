---
name: DeepSeek_point_table_protocol
description: >
  储能电站通信协议与点表集成专业Skill。依据设备清单自动生成Modbus RTU/TCP、
  IEC 60870-5-104、MQTT、OCPP 1.6J四大主流通信协议的完整点表，覆盖PCS、
  BMS、多功能电表、光伏逆变器、充电桩、柴油发电机、温控/消防/环境监测等全部
  储能系统设备。提供精确的Modbus寄存器地址映射、缩放因子、数据类型定义，
  IEC104 ASDU/COT/IOA配置规范，MQTT Topic命名层级，OCPP点表映射。
  融合并网接入报告中的调度接口IEC104详细配置与AGC/AVC指令帧结构，
  支持EMS/SCADA系统集成与对点调试场景，确保点表规范统一，对点调试一次通过率>90%。
  参考标准：IEC 60870-5-104、Modbus Application Protocol V1.1b、OCPP 1.6J Edition 2。
---

# 通信协议与点表集成专业Skill v2.0 (DeepSeek Edition)

## 目录

- [一、协议选型决策与基础规范](#一协议选型决策与基础规范)
- [二、Modbus RTU/TCP完整点表框架](#二modbus-rtutcp完整点表框架)
- [三、PCS变流器Modbus点表(全量精确地址)](#三pcs变流器modbus点表全量精确地址)
- [四、BMS电池管理系统Modbus点表(全量精确地址)](#四bms电池管理系统modbus点表全量精确地址)
- [五、多功能电表与关口表Modbus点表](#五多功能电表与关口表modbus点表)
- [六、光伏逆变器Modbus点表](#六光伏逆变器modbus点表)
- [七、充电桩双协议点表(Modbus+OCPP 1.6J)](#七充电桩双协议点表modbusocpp-16j)
- [八、柴油发电机Modbus点表](#八柴油发电机modbus点表)
- [九、辅助系统Modbus点表(消防/温控/环境)](#九辅助系统modbus点表消防温控环境)
- [十、IEC 60870-5-104调度协议点表](#十iec-60870-5-104调度协议点表)
- [十一、MQTT物联网协议点表与Topic规范](#十一mqtt物联网协议点表与topic规范)
- [十二、OCPP 1.6J充电桩完整点表映射](#十二ocpp-16j充电桩完整点表映射)
- [十三、Modbus地址空间分配总表](#十三modbus地址空间分配总表)
- [十四、数据类型转换与缩放因子规范](#十四数据类型转换与缩放因子规范)
- [十五、EMS配置模板(YAML)](#十五ems配置模板yaml)
- [十六、点表Excel生成规范](#十六点表excel生成规范)
- [十七、对点调试检查清单与故障排查](#十七对点调试检查清单与故障排查)
...
设备类型与通信需求：
  ...
  │   ├── 距离<15m → RS485 (Modbus RTU)，波特率9600~38400，8E1
  │   ├── 距离>15m 且 支持以太网 → Modbus TCP，端口502
  │   └── 有光纤接口 → Modbus TCP over Fiber
  │
  ├── 调度/电网SCADA
  │   └── → IEC 60870-5-104 (国内调度标准)，端口2404
  │
  ├── 云平台/APP
  │   └── → MQTT (物联网标准协议)，端口1883/8883(TLS)
  │
  ├── 充电桩
  │   ├── 桩控 → OCPP 1.6J / OCPP 2.0.1 (WebSocket)
  │   └── EMS监控 → Modbus TCP (补充本地监控)
  │
  └── 第三方系统对接(通过EMS中转)
...
| 协议 | 速率规范 | 响应时间要求 | 超时设置 | 典型应用 |
|------|---------|-------------|---------|----------|
| Modbus RTU | 9600/19200/38400 bps, 8E1/8N1 | ≤ 200ms | 500ms | 就地设备层 |
| Modbus TCP | 100Mbps Ethernet | ≤ 100ms | 500ms | 站内LAN设备 |
| IEC 104 | 100Mbps Ethernet, 端口2404 | ≤ 1s(总召唤) | 30s(t1) | 调度SCADA |
| MQTT | 任意, QoS 1 | ≤ 2s | 60s(keepalive) | 云平台上报 |
| OCPP 1.6J | WebSocket | ≤ 5s | 30s | 充电桩交互 |
| CAN 2.0B | 250/500kbps | ≤ 50ms | 100ms | BMS内部通信 |
...
## 三、PCS变流器Modbus点表(全量精确地址)

### 3.1 PCS遥测点(YC) — Input Registers 30001~30046
...
| 15 | 直流侧电压 | 30018 | uint16 | V | ×0.1 | 直流母线电压 |
...
### 3.2 PCS遥信点(YX) — Discrete Inputs 10001~10020
...
### 3.3 PCS遥控/遥调点(YK/YT) — Coils 00001~00005 + Holding Regs 40001~40005

| 序号 | 点名 | 地址 | 类型 | 读写 | 说明 |
|------|------|------|------|------|------|
| 1 | 启停控制 | 00001 | coil | RW | 0=停机, 1=启动 |
| 2 | 紧急停机 | 00002 | coil | RW | 0=正常, 1=急停(保持) |
| 3 | 故障复位 | 00003 | coil | WO | 脉冲: 0→1→0 |
| 4 | 并网/离网模式切换 | 00004 | coil | RW | 0=离网, 1=并网 |
| 5 | 交流断路器遥控 | 00005 | coil | RW | 0=分闸, 1=合闸 |
| 6 | 有功功率设定 | 40001 | int16 | RW | 范围: -Pmax~+Pmax (kW) |
| 7 | 无功功率设定 | 40002 | int16 | RW | 范围: -Qmax~+Qmax (kvar) |
| 8 | 功率因数设定 | 40003 | uint16 | RW | 范围: -1000~1000 (-1.000~1.000) |
| 9 | 充电最大功率限值 | 40004 | uint16 | RW | kW (充电方向) |
| 10 | 放电最大功率限值 | 40005 | uint16 | RW | kW (放电方向) |

---

## 四、BMS电池管理系统Modbus点表(全量精确地址)

### 4.1 BMS系统级遥测点(BCMU/BAMU汇总) — Input Registers 31001~31023
...
### 4.2 BMS簇级遥测点(BCMU) — 基地址31100, 每簇30个寄存器
...
### 4.3 BMS遥信/报警点 — Discrete Inputs 11001~11025
...
### 5.1 多功能电表遥测点 — Input Registers 32001~32044
...
### 5.2 电表遥信点 — Discrete Inputs 12001~12005
...
### 6.1 光伏逆变器遥测点 — Input Registers 33001~33045
...
| 6 | 直流输入功率PV2 | 33007 | uint32 | kW | ×0.001 | |
| 7 | 直流输入电压PV3 | 33009 | uint16 | V | ×0.1 | |
| 8 | 直流输入电流PV3 | 33010 | uint16 | A | ×0.01 | |
| 9 | 直流输入功率PV3 | 33011 | uint32 | kW | ×0.001 | |
| 10 | 直流输入电压PV4 | 33013 | uint16 | V | ×0.1 | |
| 11 | 直流输入电流PV4 | 33014 | uint16 | A | ×0.01 | |
| 12 | 直流输入功率PV4 | 33015 | uint32 | kW | ×0.001 | |
| 13 | 交流输出有功功率 | 33025 | int32 | kW | ×0.001 | |
| 14 | 交流输出无功功率 | 33027 | int32 | kvar | ×0.001 | |
| 15 | 交流A相电压 | 33029 | uint16 | V | ×0.1 | |
| 16 | 交流A相电流 | 33030 | uint16 | A | ×0.01 | |
| 17 | 交流B相电压 | 33031 | uint16 | V | ×0.1 | |
| 18 | 交流B相电流 | 33032 | uint16 | A | ×0.01 | |
| 19 | 交流C相电压 | 33033 | uint16 | V | ×0.1 | |
| 20 | 交流C相电流 | 33034 | uint16 | A | ×0.01 | |
| 21 | 今日发电量 | 33040 | uint32 | kWh | ×0.1 | |
| 22 | 累计发电量 | 33042 | uint32 | kWh | ×1 | |
| 23 | 逆变器效率 | 33044 | uint16 | % | ×0.1 | |
| 24 | 逆变器内部温度 | 33045 | int16 | °C | ×0.1 | |

### 6.2 逆变器遥信点 — Discrete Inputs 13001~13010

| 序号 | 点名 | 地址 | 类型 | 说明 |
|------|------|------|------|------|
| 1 | 运行状态 | 13001 | bit | 0=待机,1=并网,2=故障 |
| 2 | 电网电压异常 | 13002 | bit | |
| 3 | 电网频率异常 | 13003 | bit | |
| 4 | 直流过压 | 13004 | bit | |
| 5 | 绝缘阻抗低 | 13005 | bit | |
...
## 七、充电桩双协议点表(Modbus+OCPP 1.6J)

### 7.1 充电桩Modbus遥测点 — Input Registers 34001~34018
...
### 7.2 充电桩遥控点 — Coils 00041~00042 + Holding Regs 40041

| 序号 | 点名 | 地址 | 类型 | 读写 | 说明 |
|------|------|------|------|------|------|
| 1 | 远程启停 | 00041 | coil | RW | 0=停止, 1=启动 |
| 2 | 故障复位 | 00042 | coil | WO | 脉冲 |
| 3 | 远程功率限制 | 40041 | uint16 | RW | kW, 0=不限制 |

---

## 八、柴油发电机Modbus点表

### 8.1 柴油发电机遥测点 — Input Registers 35001~35020
...
| 序号 | 点名 | 地址 | 类型 | 读写 | 说明 |
|------|------|------|------|------|------|
| 1 | 远程启停 | 00051 | coil | RW | 0=停止, 1=启动 |
| 2 | 远程模式切换 | 00052 | coil | RW | 0=自动, 1=手动 |
| 3 | 故障复位 | 00053 | coil | WO | 脉冲 |

柴发遥信点(15001~15010): 运行/停机, 自动/手动模式, 综合报警/故障, 机油压力低, 冷却液温度高, 燃油液位低, 超速/欠速, 过载, 启动失败, 电池电压低

---

## 九、辅助系统Modbus点表(消防/温控/环境)

### 9.1 消防主机点表 — Input Registers 36001~36004 + Discrete Inputs 16001~16008
...
| 5 | 消防系统状态 | 16001 | bit | 0=正常,1=报警 |
| 6 | 烟感报警 | 16002 | bit | |
| 7 | 温感报警 | 16003 | bit | |
| 8 | 可燃气体报警 | 16004 | bit | |
| 9 | 消防灭火装置启动 | 16005 | bit | 灭火剂已释放 |
| 10 | 消防故障 | 16006 | bit | |
| 11 | 消防联动输出 | 16007 | bit | FSS跳闸信号已发出 |
| 12 | 手动报警按钮 | 16008 | bit | |

### 9.2 液冷机组点表 — Input Registers 37001~37004 + Discrete Inputs 17001~17006
...
| 9 | 高压/低压报警 | 17005 | bit | | |
| 10 | 通信故障 | 17006 | bit | | |
| 11 | 远程启停 | 00061 | coil | RW | |
| 12 | 温度设定 | 47001 | uint16 | °C | ×0.1, 目标出水温度 |

### 9.3 环境监测点表 — Input Registers 38001~38005 + Discrete Inputs 18001~18003
...
| 48 | 设点命令_归一化值 | QOS | 6(激活) | 归一化设点命令 |
| 50 | 设点命令_标度化值(遥调) | QOS | 6(激活) | 有功/无功设定(AGC推荐) |
| 100 | 总召唤命令 | QOI | 6,7 | 初始化总召唤 |
| 103 | 时钟同步命令 | CP56Time2a | 6 | GPS对时 |

### 10.3 IEC104信息对象地址(IOA)分配体系
...
### 10.4 IEC104调度遥测配置表(示例)

| 序号 | 信息点名称 | IOA | 数据类型 | 单位 | 更新周期 | 说明 |
|------|-----------|-----|---------|------|---------|------|
| 1 | 有功功率 | 0x000001 | M_ME_NC_1 | 0.1MW | 1s | 储能充放电功率 |
| 2 | 无功功率 | 0x000002 | M_ME_NC_1 | 0.1Mvar | 1s | |
| 3 | 累计充电量 | 0x000003 | M_ME_NC_1 | 0.1MWh | 60s | |
| 4 | 累计放电量 | 0x000004 | M_ME_NC_1 | 0.1MWh | 60s | |
| 5 | 并网点电压Uab | 0x000005 | M_ME_NC_1 | 0.1V | 1s | |
| 6 | 并网点电压Uc | 0x000006 | M_ME_NC_1 | 0.1V | 1s | |
| 7 | 频率 | 0x000007 | M_ME_NC_1 | 0.01Hz | 1s | |
| 8 | PCS运行状态字 | 0x000008 | M_SP_NA_1 | — | 变化时 | 位映射 |
| 9 | BMS SOC | 0x000009 | M_ME_NA_1 | % | 10s | 0~100% |
| 10 | 可用容量 | 0x00000A | M_ME_NC_1 | 0.1MWh | 10s | 当前可调度容量 |

### 10.5 IEC104调度遥控/遥调配置表

| 序号 | 控制对象 | IOA | 类型 | 说明 |
|------|---------|-----|------|------|
| 1 | 并网断路器合闸 | 0x000201 | C_SC_NA_1 | 调度遥控合闸 |
| 2 | 并网断路器分闸 | 0x000202 | C_SC_NA_1 | 调度遥控分闸 |
| 3 | AGC使能/停用 | 0x000203 | C_SC_NA_1 | 调度遥控 |
| 4 | AVC使能/停用 | 0x000204 | C_SC_NA_1 | 调度遥控 |
| 5 | 紧急停机遥控 | 0x000205 | C_SC_NA_1 | 调度紧急停机 |
| 6 | AGC功率指令(设点) | 0x000101 | C_SE_NC_1 | 调度下发功率目标值 |
| 7 | 无功功率设点 | 0x000103 | C_SE_NC_1 | 单位:0.1Mvar |
| 8 | 功率因数设点 | 0x000104 | C_SE_NA_1 | 归一化值 |

### 10.6 AGC功率指令帧结构详解

**设点命令(C_SE_NC_1)下行帧示例**:
```
68 1F          # APDU长度(31字节)
...
| 指标 | 要求值 | 说明 |
|------|--------|------|
| 指令接收延迟 | ≤ 1s | 调度主站下发到储能就地控制收到 |
| 功率响应时间 | ≤ 4s | 收到指令后功率开始变化 |
| 功率变化率 | ≤ 额定功率20%/min | 可通过调度协议约定更大值 |
| 稳态误差 | ±2% | 实际功率与目标值偏差 |
...
| 功率因数设点 | C_SE_NA_1(类型48) | 0x000104 | 归一化值, 8000=0.8 |
| AGC使能 | C_SC_NA_1(类型45) | 0x000102 | 0=停用 1=启用 |
...
调度数据网通道要求:
  ...
  - 迂回路由: 主通道故障备通道自动切换, 切换时间≤50ms
  - 通信规约: IEC104(主)+IEC104(备)或DNP3.0(备)
  - 网络地址: 调度数据网使用10.x.x.x或192.168.x.x私网地址段

纵向加密装置配置要求:
  - 加密算法: 国密算法(SM1/SM2/SM3/SM4)
  - 部署位置: 储能电站侧安装在调度数据网入口处
  - 密钥管理: 由电网调度机构统一下发
  - 隧道建立: 与调度主站建立IPSec VPN隧道, 端口500/UDP
  - 拓扑: 调度主站→调度数据网(纵向加密隧道)→[纵向加密装置]→本地SCS/远动装置→并网柜保护测控单元
```
...
| 对时方式 | 实现方式 | 精度 | 说明 |
|---------|---------|------|------|
| NTP | 主备双机配置, 从调度主站NTP服务器对时 | ±50ms | 远动装置和通信设备 |
| IRIG-B | 专用时钟装置IRIG-B接口对时 | ±1ms | 保护装置(精度要求高) |
| GPS/北斗 | 场站安装GPS/北斗双模时钟天线 | ±1ms | 必备设备, 为IRIG-B提供时间源 |

---

## 十一、MQTT物联网协议点表与Topic规范
...
## 十二、OCPP 1.6J充电桩完整点表映射

### 12.1 OCPP 1.6J核心消息映射

| OCPP消息 | 方向 | 用途 | EMS映射 |
|----------|------|------|---------|
| BootNotification | CP→CSMS | 充电桩上线注册 | 记录上线状态 |
| Heartbeat | CP→CSMS | 心跳30s | 在线监测 |
...
### 13.1 Holding Registers (40001-49999) 读写数据区
...
### 13.2 Input Registers (30001-39999) 只读数据区

| 地址范围 | 设备 | 说明 |
|---------|------|------|
| 30001-30099 | PCS1 遥测 | |
| 30100-30199 | PCS2 遥测 | |
| 31001-31399 | BMS1 遥测 | 含系统级+簇级 |
| 31400-31799 | BMS2 遥测 | |
...
### 13.4 Discrete Inputs (10001-19999) 遥信区
...
### 14.1 Modbus寄存器数据类型编码

| 数据类型 | 寄存器数 | 范围 | 说明 |
|---------|---------|------|------|
| uint16 | 1 | 0-65535 | 无符号16位整数 |
| int16 | 1 | -32768~+32767 | 有符号16位整数(补码) |
| uint32 | 2 | 0-4294967295 | 高字在前/低字在前 |
| int32 | 2 | -2147483648~+2147483647 | 补码 |
| float32 | 2 | IEEE 754单精度 | 高字在前/低字在前 |
| string | N | ASCII编码 | 每寄存器2字符 |
...
**字节序排查步骤**:
1. 读一个已知值的寄存器(如额定电压)
2. 尝试变换字节序排列: AB CD / CD AB / BA DC / DC BA
3. 与实际值对比, 确定正确排列

### 17.4 RS485/CAN现场测量参数
